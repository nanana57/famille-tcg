/* ===========================================================
   FAMILLE TCG — Multijoueur Firebase (v3 — sync simplifiée)
   =========================================================== */
...
let monRole = null;
let refPartie = null;
let refPartieData = null;    // ref vers parties_data/<partieId> (source de vérité)
let dejaLancee = false;
let defiEnCours = null;
let ecoutePrincipale = null; // handler unique pour la partie
...
function initFirebase() {
    ...
    // Écoute des notifications personnelles (acceptation, forfait)
...
function accepterDefi() {
    ...
    // On notifie les deux joueurs (chacun dans son noeud)
...
function ecouterPartie(partieId) {
    ...
    if (ecoutePrincipale) return;

    refPartieData = fbDB.ref('parties_data/' + partieId);

    // Marque ma présence (au cas où l'autre arrive en premier)
    refPartieData.child('presence/' + monId).set(true);
    refPartieData.child('presence/' + monId).onDisconnect().remove();

    // ⚠️ UN SEUL écouteur sur toute la partie_data
    ecoutePrincipale = refPartieData.on('value', snap => {
        const p = snap.val();
        ...
        // Détermine mon rôle à partir de la structure partagée
        const roleLocal = p.roles[monId] || ((monId === ids[0]) ? 'joueur1' : 'joueur2');
        monRole = roleLocal;

        // Étape 1 : lancer la partie locale une seule fois
        if (!dejaLancee) {
        ...
        // Étape 3 : appliquer l'état adverse si publié et pas par nous
...
function signalerMulliganPret() {
    ...
    // On écrit dans le chemin partagé, à notre ID
    window.multiPartie.refPartieData.child('mulligan/' + monId).set(true);
    // Le reste est géré par l'écouteur principal (voir ecouterPartie)
}
...
function appliquerEtatAdverse(etat) {
    ...
    if (etat.tourActuel === 'bot') {
        // L'adversaire vient de finir son tour → à moi
        modeAttente = false;
        tourActuel = 'joueur';
        fermerAttente();
