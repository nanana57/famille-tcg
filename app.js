...
function jouerCarte(side, index, cible) {
    ...
    recalcAuras();
    nettoyerMorts();
    setTimeout(() => { rafraichirJeu(); verifierFin(); }, 60);
}
...
function clicHeroAdverse() {
    ...
    if (tourActuel !== 'joueur' || !selection) return;
    if (B.plateau.some(x => x.motsCles.includes('Provocation'))) return info('Une Provocation protège le bot.');
...
/* Progression du mana :
   - Premier joueur  : 2 au tour 1, puis 3, 4, 5… (max 10)
   - Second joueur   : 3 au tour 1, puis 4, 5, 6… (max 10) */
...
function rafraichirJeu() {
    ...
    // Main du bot — révélation possible via sort, zoom activé
    ...
    // Plateau joueur — zoom activé (clic droit + double-clic)
