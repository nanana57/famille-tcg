...
function validerMulligan(auto) {
    ...
    // ⚠️ CORRECTION : en multijoueur, on affiche l'attente AVANT d'envoyer le signal Firebase.
    // Sinon, l'écouteur on('value') déclenché par l'écriture Firebase peut fermer l'overlay
    // puis le code reprend et réaffiche l'attente par-dessus le jeu.
    if (modeEnLigne) {
        mulliganValide = true;

        // 1) Affiche le statut d'attente D'ABORD
        afficherAttente("En attente de l'adversaire", "Ton adversaire prépare sa main…");

        // 2) Envoie le signal à Firebase ENSUITE
