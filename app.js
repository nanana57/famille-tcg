/* ===========================================================
   FAMILLE TCG — moteur de jeu (Édition Ultime Multi-Raretés)
   =========================================================== */

/* ---------- 1. Base de cartes ---------- */
function C(id, prenom, famille, cout, atk, vie, rareteDefaut, desc, motsCles, emoji) {
    return { id, prenom, famille, cout, atk, vie, rarete: rareteDefaut, desc, motsCles: motsCles || [], emoji };
}
...
/* ---------- 3. Collection, Progression & Decks (Raretés multiples) ---------- */
let collectionJoueur = {}; 
// Désormais collectionJoueur ressemble à : { 'm1': { commune: 0, rare: 0, epique: 0, legendaire: 1, fusion: 0 }, ... }
...
function getHighRarity(id) {
    ...
    return defCarte(id).rarete; // Fallback
}

function normalizeDeck(d) {
    if(d.cartes && d.cartes.length > 0 && typeof d.cartes[0] === 'string') {
        d.cartes = d.cartes.map(c => ({ id: c, rarete: defCarte(c).rarete }));
    }
}
...
function attribuerDeckDepart() {
    ...
    precon.cartes.forEach(c => {
        initColl(c.id);
        collectionJoueur[c.id].commune = 2; // Donne 2 exemplaires en commune
    });
...
function afficherBoutique(critere) {
    ...
    liste.forEach(c => {
        ...
        if (total === 0) el.classList.add('missing'); 
...
function trierCollection(critere) {
    ...
    liste.forEach(c => {
        ...
        const el = creerHTMLCarte(c, 'collection', { qty:total, enDeck:dansDeck || undefined, overrideRarete: highestRarity });
...
function ouvrirDetailCarte(idCarte) {
    ...
    function render() {
        ...
        const inDeckDb = document.getElementById('tab-decks').classList.contains('hidden') === false;
        ...
        let rowsHtml = rList.map(r => {
            ...
            const prixV = prix / 2;
            const dansDeck = inDeckDb ? tempDeckCartes.filter(x => x.id === idCarte && x.rarete === r).length : 0;
            
            let deckBtns = '';
            if(inDeckDb && deckEnEdition !== null) {
                deckBtns = `
            ...
            <div class="rarity-row">
                <div>
                    ...
                    <div style="font-size:12px; color:var(--texte-doux)">Possédé : ${possede} ${inDeckDb && dansDeck>0 ? `(Dans deck: ${dansDeck})` : ''}</div>
                </div>
        ...
        content.innerHTML = `
            ...
            <div class="detail-panel-right">
                ...
                ${inDeckDb && deckEnEdition !== null ? `<h4 style="text-align:center; color:var(--laiton-clair); margin:0 0 10px;">Édition de : ${mesDecks[deckEnEdition].nom} (${tempDeckCartes.length}/20)</h4>` : ''}
                ${rowsHtml}
            </div>
        `;
    ...
    window.acheterCarte = function(id, r, prix) {
        if (profil.coins >= prix) {
            ...
            if(document.getElementById('tab-boutique').classList.contains('active')) afficherBoutique(triBoutiqueCourant); 
            else trierCollection(triCourant);
        }
    };
    ...
    window.ajouterAuDeck = function(id, r) {
        if (tempDeckCartes.length < 20) {
            ...
            render(); afficherDeckEnCours(); trierCollection(triCourant);
        }
    };
...
function afficherDeckEnCours() {
    ...
    // Regrouper par ID et par Rareté
    const compte = {}; 
