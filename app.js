...
const dbCartes = [
    ...
    C('m6','Anness','Meridja',2,3,2,'commune','Fougue de la jeunesse meridiste.',[],'👦🏻'),
    C('m7','Abder','Meridja',2,3,2,'commune','Toujours prêt à rigoler.',[],'👦🏽'),
    ...
    C('ka10','Kamel','Kadour',2,3,2,'commune','Grand protecteur de Camilla.',[],'👦🏻'),
...
const POUVOIRS = {
    ...
    ka11:{ mode:'eclair', cible:{camp:'allie', hero:true, texte:'Choisis une cible à soigner'}, jouer:({moi, source, cible}) => {
            const v = lancerDe();
            soigner(cible, v);
            if (moi.plateau.some(x => x.id === 'ka3') && moi.plateau.some(x => x.id === 'ka4')) buff(source, 2, 2);
        } },

    ma11:{ mode:'eclair', cible:{camp:'ennemi', hero:true, texte:'Choisis une cible à frapper'}, jouer:({moi, source, cible}) => {
            const v = lancerDe();
            fraper(cible, v);
            if (moi.plateau.some(x => x.id === 'ma3') && moi.plateau.some(x => x.id === 'ma4')) buff(source, 2, 2);
        } },
    m11:{ mode:'eclair', jouer:({moi, source}) => {
            const v = lancerDe();
            if (v % 2 === 0) piocher(moi, 1); else moi.manaActuel += 1;
            if (moi.plateau.some(x => x.id === 'm4') && moi.plateau.some(x => x.id === 'm5')) buff(source, 2, 2);
        } },
    m12:{ mode:'eclair', jouer:({moi, ennemi, source}) => {
            const v = lancerDe();
            degatsHero(ennemi, v);
            if (moi.plateau.some(x => x.id === 'm4') && moi.plateau.some(x => x.id === 'm5')) buff(source, 2, 2);
        } },
    ...
    s5: { mode:'eclair', cible:{camp:'ennemi', texte:'Force une créature à frapper un allié'}, jouer:({cible, ennemi}) => {
            if (!cible) return;
            const victime = hasard(ennemi.plateau.filter(m => m !== cible));
            if (victime) echangeDegats(cible, victime);
        } },
    ...
    s20:{ mode:'eclair', cible:{camp:'allie', texte:'Donne Charge'}, jouer:({cible}) => {
            if (!cible) return;
            if (!cible.motsCles.includes('Charge')) cible.motsCles.push('Charge');
            cible.malade = false; fxSur(cible, 'Charge !', 'buff');
        } },
    ...
    s24:{ mode:'eclair', cible:{camp:'tous', texte:'Échange attaque et vie'}, jouer:({cible}) => {
            if (!cible) return;
            const a = cible.atk, v = cible.vie;
            cible.atk = Math.max(0, v);
            cible.vie = Math.max(1, a);
            cible.vieMax = cible.vie;
            fxSur(cible, '🔀', 'buff');
        } },
...
// ⬇️ NOUVEAU : flag pour savoir si on est en partie en ligne
...
function changerEcran(id) {
    ...
    const btnForfaitNav = document.getElementById('btn-forfait');
    const btnForfaitIngame = document.getElementById('btn-forfait-ingame');

    if (btnForfaitNav) btnForfaitNav.hidden = (id !== 'game-screen' || partieFinie);
    if (btnForfaitIngame) btnForfaitIngame.hidden = (id !== 'game-screen' || partieFinie);
...
function creerHTMLCarte(c, ctx, opts) {
    ...
    const badge = mode ? `<div class="power-badge ${mode}" title="${mode === 'eclair' ? 'Pouvoir immédiat' : 'Pouvoir permanent'}">${mode === 'eclair' ? '⚡' : '♾️'}</div>` : '<div class="power-badge" style="opacity:0"></div>';
    ...
    w.innerHTML = `
        ...
        <div class="card-inner">
            <div class="card bg-${c.famille} border-${c.rarete}">
                <div class="card-head">
...
document.addEventListener('keydown', e => {
    if (e.key === 'Escape') { fermerZoom(); annulerCiblage(); }
});
...
function chargerListeDecks() {
    ...
    mesDecks.forEach((d, i) => {
        ...
        const supprBtn = d.base ? '<span class="base-tag">Officiel</span>' : `<button class="del-btn" title="Supprimer ce deck" onclick="supprimerDeck(${i}, event)">✕</button>`;
...
function trierCollection(critere) {
    ...
    const ordreFamille = { Meridja:1, Marouf:2, Kerkache:3, Kadour:4, Neutre:5, Terrain:6, Sort:7 };
...
function sauvegarderDeck() {
    ...
    chargerListeDecks();
    flashInfo(tempDeckCartes.length === 20 ? 'Deck enregistré.' : `Deck enregistré — il manque ${20 - tempDeckCartes.length} carte(s) pour jouer.`);
}
...
function lancerPartie() {
    ...
    const TAILLE_MAIN_DEPART = 4;
    for (let k = 0; k < TAILLE_MAIN_DEPART; k++) piocher(B, 1);
    for (let k = 0; k < TAILLE_MAIN_DEPART; k++) if (J.deck.length) J.main.push(J.deck.shift());
...
/* ⬇️ NOUVEAU : lancement d'une partie en ligne */
function lancerPartieMultijoueur(pseudoAdversaire) {
    partieFinie = false; selection = null; ciblage = null;
    modeEnLigne = true;
    ...
    const deckJoueur = piocherDeckEnLigne();
    J.deck = deckJoueur.map(id => instancier(defCarte(id), 'J'));
    melanger(J.deck);
    ...
    const jeCommence = Math.random() > 0.5;
    J.premier = jeCommence; B.premier = !jeCommence;
    J.manaMax = 0; J.manaActuel = 0; J.numTour = 0;
    B.manaMax = 0; B.manaActuel = 0; B.numTour = 0;
    tourActuel = jeCommence ? 'joueur' : 'bot';
...
/* Récupère le deck sélectionné (ou aléatoire si aucun) pour le multijoueur */
...
function validerMulligan() {
    ...
    else if (!modeEnLigne) jouerTourBot();
...
function transformerEn(m, nom, atk, vie, emoji, motsCles) {
    m.prenom = nom; m.emoji = emoji;
    m.atk = atk; m.vie = vie; m.vieMax = vie;
    m.motsCles = motsCles || []; m.silence = true; m.auraAtk = 0;
    m.desc = 'Transformé.';
    fxSur(m, emoji, 'buff');
}
function lancerDe() {
...
function clicCarteMain(index) {
    if (tourActuel !== 'joueur' || partieFinie) return;
    if (ciblage) return;
...
function finDeTour() {
    ...
    if (modeEnLigne) {
        // En ligne : on passe le tour à l'adversaire et on publie l'état
        tourActuel = 'bot';
...
async function jouerTourBot() {
    ...
    for (const m of [...B.plateau]) {
        ...
        else if (J.plateau.length && Math.random() > 0.45) {
            cible = [...J.plateau].sort((a, b) => atkTot(b) - atkTot(a))[0];
        } else cible = J;
...
function rafraichirJeu() {
    ...
    J.main.forEach((c, i) => {
        ...
        if (tourActuel === 'joueur' && J.manaActuel >= cout && placePlateau) el.classList.add('jouable');
...
function declarerForfait() {
    ...
    if (!document.getElementById('game-screen').classList.contains('active')) {
        return flashInfo('Tu n\'es pas en combat.');
    }
    ...
    if (window.multiPartie && window.multiPartie.active) {
