...
/* ANTI-PLANTAGE SAUVEGARDE : Force le format ID simple pour les decks */
function sanitizeDecks() {
    mesDecks.forEach(d => {
        d.cartes = d.cartes.map(c => typeof c === 'string' ? c : c.id).filter(id => defCarte(id));
    });
}
...
function zoomCarte(event, id) { 
    ...
    const box = document.getElementById('card-zoom-container'); box.innerHTML = ''; box.appendChild(creerHTMLCarte(c, 'zoom')); document.getElementById('card-zoom-overlay').classList.add('open'); ajusterTextes(box); 
}
...
function editerDeck(i) {
    ...
    tempDeckCartes = tempDeckCartes.map(c => typeof c === 'string' ? c : c.id).filter(id => defCarte(id)); // Sanitize
...
function trierCollection(critere) {
    ...
    liste.forEach(c => {
        ...
        const missing = possede === 0;
        
        const el = creerHTMLCarte(c, 'collection', { qty:possede, enDeck:dansDeck || undefined, missing: missing, inDeck: dansDeck > 0 });
...
function ouvrirDetailCarte(id) {
    ...
    // Fallback CSS for modal in JS just in case
